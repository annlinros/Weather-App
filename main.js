...
function getWeather() {
  ...
  const weatherApi = `https://api.openweathermap.org/data/2.5/weather?q=${searchItem}&APPID=${apiKey}&units=${units}`;
...
function displayWeather(weather) {
  document.querySelector(
    ".output"
  ).innerHTML = `<div class="weather_results">                                             
                                                        <h2 class="city">${
                                                          weather.name
                                                        }, ${
    weather.sys.country
                                                        ...
                                                        <div class="weather_details">
                                                            ...
                                                            <img class="weather_icon" src= ${weatherIcon(
                                                              weather
                                                            )}>
                                                        </div>
                                                    </div>
...
function displayForecast(forecast) {
  ...
  document.querySelector(".forecast_output").innerHTML = days
    .map(
      day =>
        `<div class="forecast_results">
                                                                            <p class="forecast_day">${day.dt_txt.slice(
                                                                              8,
                                                                              10
                                                                            )}th</p>
                                                                            <p class="forecast_temp">${Math.round(
                                                                              day
                                                                                .main
                                                                                .temp
                                                                            )}°C</p>
...
function invalidInput() {
  document.querySelector(
    ".output"
  ).innerHTML = `<p class="alert"> Sorry! City is not found.`;
  document.querySelector(".forecast_search").style.display = "none";
  document.querySelector(".forecast_output").innerHTML = "";
}
